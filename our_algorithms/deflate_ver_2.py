...
def deflate_compress(input_file_path: str, output_file_path: str):
    ...
    print("Running LZ77 compression...")
    lz = LZ77()
    tokens = lz.compress(data)
    print(f"LZ77: {len(tokens)} tokens")
    ...
    print(f"Compressed: {compressed_size} bytes ({ratio:.1f}% of original)")
...
def deflate_decompress(input_file_path: str, output_file_path: str):
    ...
    print(f"Expected output size: {original_size} bytes")
    ...
    if len(output) != original_size:
        print(f"Warning: decompressed size {len(output)} != expected {original_size}")
